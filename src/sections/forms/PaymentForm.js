...
const PaymentForm = ({ selectedPlan, onClose }) => {
  ...
  const getBillingAddressInfo = () => {
    getBillingAddressDetails().then((response) => {
...
const handleSubmit = async (event) => { //eslint-disable-line
    ...
    if (payment_status === "succeeded") {
      if (Object.keys(response?.paymentIntent).length > 0 && response?.paymentIntent?.status === "succeeded") {
        ...
        await subscribeToPlan(params).then((response) => {
          setIsDisabled(false);
          const successMessage = response.data.message || "Subscription Successful! Thank you for choosing our service. Enjoy your new plan!";
  ...
  return (
    <>
      ...
      <MainCard
        title="Please fill card details to continue"
        sx={{ maxHeight: "80vh" }}
          ...
          <Grid container spacing={2} >
            <Grid item xs={12} >
              ...
              <Stack spacing={1.25}>
                <InputLabel
                  ...
                  sx={{ color: theme.palette.common.black }} >Address :- <span>{billingAddress?.address_line}</span>  </InputLabel>
              </Stack>
            </Grid>
            <Grid item lg={6} xs={12} md={6} >
              <Stack spacing={1.25}>
                <InputLabel
                  htmlFor="customer-name"
                  sx={{ color: theme.palette.common.black }}>City :- <span>{billingAddress?.city}</span></InputLabel>
              </Stack>
            </Grid>
            <Grid item lg={6} xs={12} md={6} >
              <Stack spacing={1.25}>
                <InputLabel
                  htmlFor="customer-name"
                  sx={{ color: theme.palette.common.black }}>State :- <span>{billingAddress?.state}</span></InputLabel>
              </Stack>
            </Grid>
            ...
            <Grid item lg={6} xs={12} md={6} >
              <Stack spacing={1.25}>
                <InputLabel
                  htmlFor="customer-name"
                  sx={{ color: theme.palette.common.black }}>Zip Code :- <span>{billingAddress?.postal_code}</span></InputLabel>
              </Stack>
            </Grid>
            <Grid item lg={6} xs={12} md={6} >
              <Stack spacing={1.25}>
                <InputLabel
                  htmlFor="customer-name"
                  sx={{ color: theme.palette.common.black }}>Phone :- <span>{billingAddress?.phone_number}</span></InputLabel>
              </Stack>
            </Grid>
            ...
            <Grid item xs={12} lg={12} >
              ...
              <Stack direction={{ xs: 'column', sm: 'row' }} justifyContent="flex-end" spacing={1.5} sx={{ mt: 3.5 }}>
                ...
                <Button type="submit" size="small" variant='contained' disabled={(disabled === true) || (Object.keys(billingAddress).length === 0)} >
